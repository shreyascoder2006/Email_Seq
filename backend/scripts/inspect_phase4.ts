...
import { runScheduler, getSchedulerQueue, startScheduler } from '../src/queues/schedulerQueue';
...
async function main() {
  ...
  // Invoke runScheduler directly to trace it in this process
  try {
    await runScheduler({ name: 'scheduler:tick' } as any);
  } catch(e) {
    console.log("runScheduler error:", e);
  }
